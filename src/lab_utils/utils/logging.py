...
Shared across every paper built on lab-utils — import it in entry points
(`train.py`, scripts) instead of gluing raw `print`/ad-hoc logging or
per-paper logging setup.
"""
...
def log_config(config: dict) -> None:
    """Log a config dict as a banner of key: value lines.

    Example:
        >>> log_config({"lr": 1e-3, "batch_size": 64})
        # Logs: "Experiment config:" then "  lr: 0.001" / "  batch_size: 64"
    """
...
def get_loguru_safe_tqdm():
    ...
    Note:
        ...
        from lab_utils.utils.logging import get_loguru_safe_tqdm
        tqdm = get_loguru_safe_tqdm()
    """
